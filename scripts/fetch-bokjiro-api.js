...
async function fetchData() {
  ...
  try {
    ...
    if (rawItems.length > 0) {
      console.log(`👀 [참고] 복지로 서버에서 방금 가져온 데이터 목록 (총 ${rawItems.length}건, 랜덤선택: ${randomPage}페이지):`);
    ...
    for (const item of freshItems) {
      if (targetItems.length < MAX_ITEMS) {
        targetItems.push({ rawItem: item, source: 'BOKJIRO' });
        existingNames.add(item.servNm || item.서비스명 || item.name);
      }
    }
  } catch (err) {
    console.error('복지로 스캔 중 에러:', err.message);
  }
  ...
  for (const target of targetItems) {
    try {
      ...
      형식: {name: 혜택명, category: '혜택', region: '전국', startDate: 'YYYY-MM-DD', endDate: 'YYYY-MM-DD', location: 소관부처명, target: 대상층, summary: 혜택 요약설명, link: 링크주소}
