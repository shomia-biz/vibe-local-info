...
2. 본문 구조
   ...
   - 본문의 소제목은 "## 1. [소제목]" 형태로 최소 4개 이상 나누어 작성하며, 목차의 링크와 소제목이 정확히 연결되게 앵커를 설정하세요.
   - 내용과 관련된 보조 이미지를 본문에 2개 더 추가하세요.
     - 두 번째 이미지는 "## 2. [소제목]" (두 번째 소제목) 섹션의 마지막에 삽입하세요. (예: ![이미지 설명](https://image.pollinations.ai/prompt/[2번 소제목과 관련된 영문키워드]?width=800&height=400&nologo=true))
     - 세 번째 이미지는 "## 4. [소제목]" (네 번째 소제목) 섹션의 마지막에 삽입하세요. (예: ![이미지 설명](https://image.pollinations.ai/prompt/[4번 소제목과 관련된 영문키워드]?width=800&height=400&nologo=true))
...
5. AI 티 제거 (Humanizer 규칙 적용)
   - 번역투("~에 대해", "~를 통해", "~할 수 있다", "~의해")나 수동태를 절대 사용하지 말고, 자연스러운 능동태 한국어로 쓰세요.
   - "결론적으로", "따라서", "혁명적", "파격적", "압도적" 등 기계적이거나 과장된 표현을 빼고 담백하게 쓰세요.
   - "~일 것이다", "~인 것이다" 등 애매한 추측성 말투 대신 확신에 찬 어조로 단언하세요.
   - 문장 길이를 다양하게 섞고, "~다"로 끝나는 문장이 3번 이상 연속되지 않게 리듬감 있게 작성하세요.
